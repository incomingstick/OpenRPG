...
int parse_args(int argc, char* argv[], int* total) {
    ...
    /* this parsing will require significant improvements
        before it is ready for v0.1.0-dev */
    if(optind < argc) {
        Die d6(atoi(argv[optind++]));
        *total = d6.roll();
        while(optind < argc) {
            if ((string) argv[optind] == "+" && argv[optind + 1]) {
                *total += d6.roll(atoi(argv[++optind]));
            } else if ((string) argv[optind] == "-" && argv[optind + 1]) {
                *total -= d6.roll(atoi(argv[++optind]));
            }
        }
    }
...
int main(int argc, char* argv[]) {
    int total, status = parse_args(argc, argv, &total);
